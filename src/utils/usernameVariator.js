...
export class UsernameVariator {
    ...
    _addPlatformSpecificVariations(username, variations) {
        ...
        const platformSuffixes = [
            ...
            'brasil', 'brazil', 'br', 'sp', 'rj', 'mg', 'rs', 'pr',
            'bh', 'sampa', 'rio', 'poa', 'nordeste', 'sul', 'sudeste',
            'carioca', 'paulista', 'gaucho', 'mineiro', 'baiano', 'nordestino',
            
            // Ocupações e nicho
            'modelo', 'model', 'atriz', 'ator', 'actress', 'actor', 'photo',
            'foto', 'sexy', 'hot', 'gamer', 'games', 'stream', 'dev', 'coach',
            'creator', 'influencer', 'digital', 'fit', 'fitness', 'musica', 'music',
            'tattoo', 'ink', 'dance', 'danca'
        ];
    ...
    _addAdultContextVariations(username, variations) {
        const adultSuffixes = [
            ...
            'adult', 'nudes', 'xxx', 'nsfw', 'lover', 'kink',
            'fetish', 'dom', 'sub', 'feet', 'paws', 'bdsm',
            'lewd', 'only', 'fans', 'fansly', 'priv', 'pvt',
            
            // Termos em português
            'sensual', 'safada', 'safado', 'gostosa', 'gostoso',
            'delicia', 'linda', 'lindo', 'tesuda', 'tesudo',
            'provocante', 'picante', 'musa', 'amor', 'prazer',
            'segredo', 'sigilo', 'privado', 'privacidade', 'especial',
            'conteudo', 'privacy', 'contato', 'atendimento', 'vip'
        ];
     ...
     * Variações de limpeza e transformações avançadas
     */
    _addCleanVariations(username, variations) {
        ...
        // Substituições de letras por números (leet speak)
        const leetMap = {
            'a': '4', 'e': '3', 'i': '1', 'o': '0', 's': '5',
            't': '7', 'b': '8', 'g': '9', 'z': '2'
        };
        
        // Conversão simples leet
        let leetUsername = '';
        for (const char of username.toLowerCase()) {
            leetUsername += leetMap[char] || char;
        }
        if (leetUsername !== username) {
        ...
        let withoutAccents = '';
        for (const char of username.toLowerCase()) {
            withoutAccents += accentMap[char] || char;
        }
        if (withoutAccents !== username) {
